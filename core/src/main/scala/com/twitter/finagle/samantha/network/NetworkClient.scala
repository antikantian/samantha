...
trait NetworkRichClient { self: Client[Command, Feedback] =>
...
object NetworkClient extends Client[Command, Feedback] with SamanthaRichClient {
  ...
  case class Client(
    ...
    protected def newDispatcher(transport: Transport[In, Out]): Service[Command, Feedback] = {
      new NetworkDispatcher(new StageTransport(transport), NetworkConfig(params))
    }
...
//      )
  ...
  val client: NetworkClient.Client = Client()
